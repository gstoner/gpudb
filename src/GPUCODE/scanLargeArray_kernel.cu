...
template <bool isNP2>
__device__ static void loadSharedChunkFromMem(int *s_data,
									   const int *g_idata, 
									   int n, int baseIndex,
									   int& ai, int& bi, 
...
template <bool isNP2>
static __device__ void storeSharedChunkToMem(int* g_odata, 
                                      const int* s_data,
                                      int n, 
                                      int ai, int bi, 
                                      int mem_ai, int mem_bi,
...
static __device__ void clearLastElement(int* s_data, 
                                 int *g_blockSums, 
                                 int blockIndex)
{
    if (threadIdx.x == 0)
...
template <bool storeSum>
static __device__ void prescanBlock(int *data, int blockIndex, int *blockSums)
{
...
static __global__ void prescan(int* d_data,
						int *g_odata, 
                        const int *g_idata, 
                        int *g_blockSums, 
                        int n, 
...
//with shared memory
template <bool storeSum, bool isNP2>
static __global__ void prescan(int *g_odata, 
                        const int *g_idata, 
                        int *g_blockSums, 
                        int n, 
                        int blockIndex, 
...
__global__ static void uniformAdd(int *g_data, 
                           int *uniforms, 
                           int n, 
                           int blockOffset, 
                           int baseIndex)
